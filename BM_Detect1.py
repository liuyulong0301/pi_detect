...
while True:
    ...
    for detection in detection_result.detections:
        ...
        X = np.random.randint(bbox.origin_x, bbox.origin_x+bbox.width, 5)
        Y = np.random.randint(bbox.origin_y, bbox.origin_y+bbox.height, 5)
        Distance = []
        for x in X:
            for y in Y:
                if x<=0:
        ...
        elif x_center>=240 and x_center<=400:
